...
class object_model:
    def __init__(self, windows_size, test=0.2, dataset="SEA", classification=True, shap_class=0, external=False):
        ...
        if self.external:
            self.ex_detector = external_detector()
...
class external_detector:
    def __init__(self, layer_name="drift"):
        self.dtypes = None
        self.model = None
        self.X_middle_train = None
        self.layer_name = layer_name

    def build_detector(self, X, y):
        # 进行数据集的归一化处理
        ...
        self.X_middle_train = self.map2layer(X.copy(), "drift")

        self.explainer = shap.GradientExplainer((self.model.get_layer("drift").input, self.model.layers[-1].output),
                                                self.map2layer(X.copy(), "drift"))

        shap_values = self.explainer.shap_values(self.map2layer(X, "drift"), nsamples=500)
    ...
    def get_shap_values(self, X):
        return self.explainer.shap_values(self.map2layer(X, "drift"), nsamples=500)
